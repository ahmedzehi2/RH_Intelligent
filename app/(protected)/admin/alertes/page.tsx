...
function analyzeRiskEmployees(
  ...
  const deptMap = new Map(depts.map((d) => [d.departement_id, d.nom_departement]))
...
function analyzeDeptRisks(
  ...
  return depts.map((d) => {
    ...
    let observation = "Aucune anomalie"
    if (retardPct >= 15) observation = `Taux retard eleve (${retardPct}%)`
    else if (deptAbs.length >= deptEmps.length) observation = `${deptAbs.length} absences detectees`
    else if (score >= 40) observation = "Plusieurs indicateurs a surveiller"

    return { dept: d.nom_departement, score, level, observation, totalEmp: deptEmps.length }
