...
import { getStatusBadgeClass, getStatusLabel, STATUS_LEGEND } from "@/lib/status-colors"
...
const validateForm = (f: PointageForm): string | null => {
  ...
  // Pour Mission et Formation, heure d'entrée optionnelle
  const typeJourSpecial = f.type_jour === "Mission" || f.type_jour === "Formation"
  if (!typeJourSpecial && !f.heure_entree) return "Heure d'entrée obligatoire"
            ...
            {filtered.map((emp, empIdx) => (
              <tr key={emp.employe_id} className={`border-b border-gray-100 ${empIdx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}`}>
                ...
                {emp.planning.slice(0, 7).map((jour, dayIdx) => {
                  ...
                  const cellClass = getStatusBadgeClass(statutAffiche)
                  ...
                  const isFormation = statutAffiche === "Formation"
                  const isMission = statutAffiche === "Mission"
                  const isConge = statutAffiche === "Conge"
                    ...
                    >
                      <div
                        ...
                        onClick={() => {
                          ...
                          } else if (statutAffiche === "Absent" || isConge || isMission || isFormation) {
                      ...
                      >
                        ...
                        {/* FORMATION sans pointage */}
                        {isFormation && !p && (
                          <div className="flex flex-col gap-1">
                            <div className="text-[13px] font-bold opacity-95">🎓</div>
                            <div className="text-[10px] font-semibold opacity-90">En formation</div>
                            <div className="text-[9px] opacity-70">Activité d&apos;apprentissage</div>
                          </div>
                        )}

                        {/* MISSION sans pointage */}
                        {isMission && !p && (
                          <div className="flex flex-col gap-1">
                            <div className="text-[13px] font-bold opacity-95">🚗</div>
                            <div className="text-[10px] font-semibold opacity-90">
                              {(jour as any).type_mission || "En mission"}
                            </div>
                            <div className="text-[9px] opacity-70">Journée travaillée</div>
                          </div>
                        )}
                        ...
                        {isConge && !p && (
                          <div className="flex flex-col gap-1">
                            <div className="text-[13px] font-bold opacity-95">🏝️</div>
                            <div className="text-[10px] font-semibold opacity-90">
                              {jour.type_conge || "En congé"}
                            </div>
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500 pt-3 border-t border-gray-100 flex-wrap">
        {STATUS_LEGEND.map(({ dot, label }) => (
          <div key={label} className="flex items-center gap-1.5">
            <span className={`w-2.5 h-2.5 rounded-full ${dot} inline-block shadow-sm`} />
  ...
  const formFields = (
    <div className="grid gap-4 py-4">
      ...
      <div>
        ...
        <Select value={form.type_jour || "auto"} onValueChange={v => updateForm("type_jour", v === "auto" ? "" : v)}>
          <SelectTrigger><SelectValue placeholder="Automatique (calculé)" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="auto">Automatique</SelectItem>
            <SelectItem value="Present">✅ Présent</SelectItem>
            <SelectItem value="Retard">⏰ Retard</SelectItem>
            <SelectItem value="Absent">❌ Absent</SelectItem>
            <SelectItem value="Conge">🏝️ Congé</SelectItem>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>{(form.type_jour === "Mission" || form.type_jour === "Formation") ? "Heure Entrée" : "Heure Entrée *"}</Label>
      ...
      {(form.type_jour === "Mission" || form.type_jour === "Formation") && (
        <div className={`text-xs px-3 py-2 rounded-lg border font-medium ${
          form.type_jour === "Formation"
            ? "bg-indigo-50 text-indigo-700 border-indigo-200"
            : "bg-purple-50 text-purple-700 border-purple-200"
        }`}>
          {form.type_jour === "Formation"
            ? "🎓 Formation — activité d'apprentissage, heures optionnelles"
            : "🚗 Mission — journée travaillée hors institution, heures comptabilisées"}
        </div>
      )}
